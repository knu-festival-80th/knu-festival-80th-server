...
| Method | Endpoint | 설명 | 인증 |
|--------|----------|------|------|
| POST | `/v1/matchings` | 매칭 신청 | 불필요 |
| POST | `/v1/matchings/result` | 매칭 결과 조회 (body로 ID+비밀번호) | 불필요 |
| DELETE | `/v1/matchings` | 매칭 신청 취소 (body로 ID+비밀번호) | 불필요 |
| GET | `/v1/matchings/status` | 매칭 서비스 상태 조회 | 불필요 |
| GET | `/v1/matchings/unmatched` | 미매칭 공개 목록 조회 | 불필요 |
