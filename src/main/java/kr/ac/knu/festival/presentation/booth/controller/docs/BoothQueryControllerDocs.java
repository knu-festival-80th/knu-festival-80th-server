...
public interface BoothQueryControllerDocs {
    ...
    @Operation(summary = "부스 목록 조회", description = "인기순(누적 대기 등록 수 많은 순), 대기 적은 순, 이름순으로 정렬된 부스 목록을 조회합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공")
    })
    ResponseEntity<ApiResponse<List<BoothListResponse>>> getBooths(
            @Parameter(description = "정렬 기준: popular(누적 대기 등록 수 DESC) / waiting-asc / name-asc") String sort
    );
    ...
    @Operation(summary = "부스 랭킹 SSE 구독", description = "인기순(누적 대기 등록 수 많은 순), 대기 적은 순, 이름순 랭킹 변경을 Server-Sent Events로 구독합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "SSE 연결 성공")
    })
    SseEmitter streamBoothRankings(
            @Parameter(description = "구독 랭킹 기준: popular(누적 대기 등록 수 DESC) / waiting-asc / name-asc") String sort
    );
}
