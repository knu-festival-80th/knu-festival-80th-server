...
public interface WaitingQueryControllerDocs {
    ...
    @Operation(summary = "현재 대기 현황 조회", description = "부스의 현재 대기팀 수를 조회합니다.")
    ResponseEntity<ApiResponse<WaitingStatusResponse>> getBoothStatus(Long boothId);

    @Operation(summary = "내 대기 상태 조회", description = "전화번호 뒤 4자리로 본인 확인 후 자신의 대기 상태를 조회합니다.")
    ...
    ResponseEntity<ApiResponse<List<WaitingResponse>>> getWaitings(Long boothId, WaitingStatus status);
}
